...
import { ThumbsUp, Bookmark, Share2, MoreVertical, MessageCircle, CheckCircle2, Clock, SquareStack, Image as ImageIcon, Smile, ArrowUpDown } from 'lucide-react';
...
export default function ArticleDetail() {
    ...
    const [sortType, setSortType] = useState<'all' | 'hot' | 'new' | 'floor'>('all');
...
永远对知识保持敬畏之心。`,
    ...
    const parseContent = (content: string) => {
        ...
        let currentSection: { type: 'title' | 'today' | 'tomorrow' | 'reflection' | 'text'; content: string[] } | null = null;
        ...
        lines.forEach((line) => {
            ...
            } else if (trimmed.includes('今天做了:') || trimmed.includes('今天做了：')) {
                if (currentSection) sections.push(currentSection);
                currentSection = { type: 'today', content: [] };
            } else if (trimmed.includes('明天计划:') || trimmed.includes('明天计划：')) {
                if (currentSection) sections.push(currentSection);
                currentSection = { type: 'tomorrow', content: [] };
            } else if (trimmed.includes('今日感悟:') || trimmed.includes('今日感悟：')) {
                if (currentSection) sections.push(currentSection);
    ...
    const handleThumb = () => {
        if (article) {
            setArticle({
                ...
                thumbNum: article.hasThumb ? article.thumbNum - 1 : article.thumbNum + 1
            });
        }
    };
    ...
    const handleFavour = () => {
        if (article) {
            setArticle({
                ...
                favourNum: article.hasFavour ? article.favourNum - 1 : article.favourNum + 1
            });
        }
    };
    ...
    if (!article) {
        return <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">加载中...</div>;
    }
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-4">
                            ...
                            <div className="flex items-start gap-3 mb-6">
                                <img 
                                    ...
                                    src={article.user.userAvatar || 'https://img.pawpaw18.cn/user-img/default-avatar.jpg'} 
                            ...
                            <div className="text-gray-800 dark:text-gray-200 mb-6">
                                {contentSections.map((section, index) => (
                                    <div key={index} className="mb-4">
                                        {section.type === 'title' && (
                                            <h1 className="text-2xl font-bold mb-4">{section.content[0]}</h1>
                                        )}
                                        {section.type === 'today' && (
                                            <div className="mb-4">
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                                <div className="flex items-center gap-4">
                                    ...
                                    >
                                        <ThumbsUp className={`w-5 h-5 ${article.hasThumb ? 'fill-current' : ''}`} />
                                    ...
                                    >
                                        <Bookmark className={`w-5 h-5 ${article.hasFavour ? 'fill-current' : ''}`} />
                                ...
                                <div className="space-y-4">
                                    {comments.length === 0 ? (
                                        ...
                                        comments.map((comment) => (
                                            <div key={comment.id} className="flex gap-3 pb-4 border-b border-gray-100 dark:border-gray-700 last:border-0">
                                                <img 
                                                    className="w-10 h-10 rounded-full object-cover flex-shrink-0" 
                                                    src={comment.user.userAvatar || 'https://img.pawpaw18.cn/user-img/default-avatar.jpg'} 
                                                ...
                                                <div className="flex-1">
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        <button className="flex items-center gap-1 text-gray-500 dark:text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors">
                                                            ...
                                                            <span className="text-sm">{comment.thumbNum}</span>
                                                        </button>
                    ...
                    <div className="w-80 flex-shrink-0 space-y-4">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
                            <div className="flex items-center justify-between mb-3">
                                <h3 className="font-semibold text-gray-900 dark:text-gray-100">热门话题</h3>
                            ...
                            <div className="flex items-start gap-3 mb-3">
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-2">
                                        <h3 className="font-bold text-gray-900 dark:text-gray-100">MOCK INTERVIEW</h3>
                                        <span className="px-2 py-0.5 text-xs bg-green-500 text-white rounded-full">1v1 模拟面试</span>
                                    </div>
                                    <p className="text-sm text-gray-700 dark:text-gray-300 mb-1">面多多 Offer多多</p>
                                    <p className="text-xs text-gray-600 dark:text-gray-400 mb-2">针对性预测面试题</p>
                                    <p className="text-xs text-gray-600 dark:text-gray-400 mb-3">详细复盘报告</p>
