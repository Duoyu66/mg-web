...
const RankingList = () => {
    ...
    },
    {
      ...
      render: (text: string, record: any, index: number) => (
        <div className="flex items-center justify-start">
          <img
            className="mr-1 w-[50px] h-[50px] rounded-full"
          ...
          <div className="flex flex-col justify-center items-start w-[calc(100%-50px)]">
            ...
            <span className="text-[12px] text-[#4c4948]">
              {" "}
              已参加竞赛 {record.examNum} 次
            </span>
          </div>
        </div>
      ),
  ...
  const rankBox = (rankList: any) => (
    <div className="relative w-[48%] border border-red-500">
      <div className="flex w-full absolute left-[-25px] top-0 text-[20px] font-semibold text-[#bdc3c7]">
        <img
          src="https://static.leetcode.cn/cn-assets/webpack_bundles/images/china_map.e6e137c72.svg"
          alt=""
          className="w-[62px] h-[30px]"
        />
        {textMap[showRankType]}
      </div>
      {rankList.length > 0 ? (
        <div>
          <div className="w-full h-[200px] flex flex-col justify-start items-center mt-[10px]">
            <div className="relative w-[90px] h-[90px] rounded-full flex justify-center items-center bg-gradient-to-b from-[#feea88] to-[#d7a807] shadow-[0_0_50px_rgba(0,0,0,0.15)]">
              <img
                className="w-[calc(100%-10px)] h-[calc(100%-10px)] rounded-full bg-white/40 text-[#373737] text-[40px] opacity-75"
                src={rankList[0]?.avatarUrl}
                alt=""
              />
              <span className="absolute bottom-[-15px] bg-white rounded-lg px-2 py-0.5 text-[#ffa116] max-w-[120px] whitespace-nowrap overflow-hidden text-ellipsis">
                {rankList[0].nickName}
              </span>
            </div>
  ...
  return (
    <div className="w-[98%] h-full flex justify-around items-center">
      <div
        style={{ display: "none" }}
        className="w-[120px] absolute right-[10px] top-[10px]"
      >
        <Dropdown
          menu={{
            items,
